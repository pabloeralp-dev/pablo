"""Options Pricer — QuantDesk page (Black-Scholes-Merton)."""
...
from styles import (
    inject_css, BG_PAGE, BG_CARD, BG_CHART, BORDER,
    TEXT_PRIMARY, TEXT_DIM, TEXT_MUTED, CALL_BLUE, PUT_RED,
    ACCENT_GOLD, ACCENT_TEAL, FONT_MONO, _rgba, _chart_base, _apply_subplot_theme,
)
...
with tab_live:
    ...
    def _fetch_live_options_spot(ticker: str):
        tk = yf.Ticker(ticker)
    ...
    with col_tc:
        ticker_input = st.text_input(
            ...
            key="options_live_ticker",
        ).strip().upper()
        if ticker_input and ticker_input != st.session_state.get("ticker", ""):
    ...
    try:
        live_spot, expiries = _fetch_live_options_spot(ticker_input)
        st.markdown(f"""
